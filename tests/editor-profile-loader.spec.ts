...
it('loads packaged filesystem search with exact cwd and pre-dispatch cancellation', async () => {
  ...
  const script = [
    ...
    "const live = await ctx.tools.execute({ callId: CallId('profile-live'), name: 'glob', arguments: { pattern: 'profile-marker.ts' }, agent, signal: new AbortController().signal })",
    'const controller = new AbortController()',
    'controller.abort()',
    "const cancelled = await ctx.tools.execute({ callId: CallId('profile-cancel'), name: 'grep', arguments: { pattern: 'profileMarker' }, agent, signal: controller.signal })",
