...
Status as of `1.0.4`: the Q1 cross-platform CI matrix, the Q4 tag-driven release workflow, and the in-repository Registry entry with the Registry's own validator run are in place; see the [Public CI and Registry Evidence Agent Note](agent-notes/2026-09-09-public-ci-and-registry-evidence.md).

Status as of `1.1.0`: Q2's pinned upstream baseline is in place. `config/upstream-baseline.json` records the reviewed official ref, `npm run test:harness` extracts its specs from that ref instead of reading the checkout's working tree, and every official spec must be classified as aligned or as a divergence with a stated reason — an unclassified or vanished spec fails as `fixture unavailable`. See the [Upstream 0.1.2-rc.1 Baseline Agent Note](agent-notes/2026-09-09-upstream-0.1.2-rc.1-baseline.md).

Status as of `1.2.0`: the pinned baseline moved to `dsh-v0.1.5-rc.1` through the review the `1.1.0` gate was built to force. The official package publishes the same spec set and the aligned `codec.spec.ts` is unchanged, so the gate passed on a bare ref bump; the runtime work was elsewhere — live streaming now comes from process-local `agent/assistant-stream` frames instead of durable per-token events, session format v0 logs migrate sideways to v3 on first read, and `check:profile` was repaired (its official reference file had moved before `0.1.2-rc.1`, so it had crashed rather than run since `1.1.0`). See the [Upstream 0.1.5-rc.1 Baseline Agent Note](agent-notes/2026-09-10-upstream-0.1.5-rc.1-baseline.md). Trusted-publishing activation, Q0, Q2's scheduled latest-upstream observation, and Q3 remain open.

Follow-up after `1.3.0`: the pinned baseline moved to `dsh-v0.1.5-rc.3` following review. The official spec set and contents remain byte-identical to `0.1.5-rc.1`; upstream changes were confined to web client feedback refinements, deliverable icon artwork, and monorepo vendor dependency pinning, leaving ACP protocol behavior and runtime semantics unchanged. See the [Upstream 0.1.5-rc.3 Baseline Agent Note](agent-notes/2026-09-10-upstream-0.1.5-rc.3-baseline.md).
