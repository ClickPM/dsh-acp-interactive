...
本项目采用“组合优先、上游贴合”的产品方向，不以追赶其他社区实现的功能数量为目标。Harness 已发布的领域插件继续拥有行为，ACP 层只承担适配、协商、投影和精确生命周期绑定；公开 CI、固定／最新上游兼容信号和真实 Zed 验证优先于新增功能。完整原则与实施顺序见[上游贴合、公开 CI 与 Zed 验证规范](upstream-alignment.md)。
