...
The standalone repository has no runtime dependency on a DeepSeek Harness checkout. For development compatibility, set `DSH_HARNESS_ROOT` to a read-only official checkout or place one at the sibling `../deepseek-harness` path, then run `npm run test:harness`. The command extracts the official ACP specs from the pinned ref recorded in `config/upstream-baseline.json` — not from whatever the checkout has checked out — into an ignored temporary directory, and runs the specs classified there as aligned against this repository's `src`. Because this server advertises more ACP capabilities than the official automation-only transport, the remaining official specs are recorded as explicit divergences with a reason each; the run prints them and fails whenever the pinned ref adds, removes, or renames a spec so the change is reviewed rather than silently skipped. `check:profile` reconciles official candidate packages, human commands, required providers, and critical consumers, reporting review-required drift without rewriting the release composition. `verify:packed` installs the tarball outside the repository and starts the real ACP launcher. `npm run test:all` chains repository tests, official compatibility tests, and profile reconciliation.
