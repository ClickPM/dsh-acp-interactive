...
`apply(ctx, config)` 需要 `agents`、`commands`、`tools`、`sessionPersistence` 和 `sessionQuery`。它只回答自己创建的 agent 的审批请求，并把外来请求交给下一个监听器。一条连接可以拥有多个互相隔离的 session。
...
当前支持文字 prompt 和直接斜杠命令。图片、resource link、音频、embedded resource、MCP server 和 additional directory 会被明确拒绝。恢复历史中出现丰富的人类、assistant 或工具结果 block 时也会失败，不会丢弃内容。模型／配置 selector、mode 和 elicitation 属于后续阶段。
...
- Config selector、协作 mode、用户问题 elicitation、MCP server 和 additional directory 延后实现。
